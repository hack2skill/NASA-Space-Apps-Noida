...
import math
from .models import *
from spacetrek.utils import render_to_pdf, createticket
...
def query(request, q):
    ...
    q = q.lower()
    for place in places:
        if (q in place.city.lower()) or (q in place.landingSite.lower()) or (q in place.code.lower()) or (q in place.planet.lower()):
            filters.append(place)
    return JsonResponse([{'code':place.code, 'city':place.city, 'country': place.planet} for place in filters], safe=False)
